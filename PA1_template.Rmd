...
Over all the observed days, the mean total number of steps per day was `r by_date_orig$daily_ttl_mean` and the median was `r by_date_orig$daily_ttl_median`.
...
Specifically, there are `r na_cnt` missing data for the number of steps taken.
...
Using the augmented data, the median number of steps taken per day is `r by_date_fixed$daily_ttl_median` and the mean value is `r by_date_fixed$daily_ttl_mean`. By filling in the missing values, the data have become strongly unimodal around the mean values, enough to bring the mean and median to almost the same value, which is in the neighborhood of the median value from the original data.
